...
Y_n = & ln(\frac{p}{1-p}) = \beta_0 + \beta_1 X_{pop} + \boldsymbol{\beta_c} poly(ChillingTime) \\
     & + \boldsymbol{\beta_i} (X_{pop} * poly(ChillingTime)) + \epsilon
\end{split}
\end{equation}
...
for (i in 1:11) {
    ...
    # Plot that pop
    title <- paste0('Population: ', i)
    plot(ChillingTime, pred, main = title, ylab = 'FittedGerminationRate')
    ideal_chill <- round(ChillingTime[which.max(pred)], 3)
    abline(v = ideal_chill, col = "red", lty = 2, lwd = 2)

    chill_ideal$IdealChillWeeks[i] <- ideal_chill
}
par(mfrow = c(1, 1))
```
...
```{r chilling_diff, fig.height = 8}
chill_diff <- as.data.frame(c(1:11))
colnames(chill_diff) <- 'Population'
chill_diff$GerminationDiff <- character(11)
par(mfrow = c(3, 4))
for (i in 1:11) {
    Population_fit <- rep(i, n)
    ChillingTime_fit <- seq(0, 12, length = n)
    df_fit <- as.data.frame(cbind(Population_fit, ChillingTime_fit))
    colnames(df_fit) <- c('Population', 'ChillingTime')
    df_fit$Population <- as.factor(df_fit$Population)
    fit_line <- predict(log_mod, df_fit, type = 'response')
    ...
    # Plot that pop
    title <- paste0('Population: ', i)
    plot(ChillingTime_fit, fit_line, main = title, ylab = 'FittedGerminationRate', xlab = 'ChillingTime')
    abline(v = 9, col = "blue", lty = 2, lwd = 2)
    abline(v = 10, col = "green", lty = 2, lwd = 2)
    abline(h = pred[1], col = "blue", lty = 2, lwd = 2)
    abline(h = pred[2], col = "green", lty = 2, lwd = 2)
}
par(mfrow = c(1, 1))
...
- Predict the impact of chilling time decreasing from 10 to 9 weeks on germination rate (mixture of icnreases and decreases, largest is a decrease)
...
```{r coefs}
coefs <- log_mod$coefficients
conf <- confint(log_mod)
coefs <- as.data.frame(cbind(round(coefs, 3), round(conf[, 1], 3), round(conf[, 2], 3)))
colnames(coefs) <- c('Estimate', 'Lower2.5', 'Upper97.5')

kable(coefs, booktabs = TRUE, longtable = TRUE)
```